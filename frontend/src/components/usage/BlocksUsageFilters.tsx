...
export interface BlocksUsageFilterValues {
  search: string;
  status: string;
  model: string;
  minCost: string;
...
interface BlocksUsageFiltersProps {
  filters: BlocksUsageFilterValues;
  appliedFilters: BlocksUsageFilterValues;
  onChange: (filters: BlocksUsageFilterValues) => void;
  onApply: (filters: BlocksUsageFilterValues) => void;
  onClear: () => void;
  availableModels: string[];
}

export function BlocksUsageFilters({
  filters,
  appliedFilters,
  onChange,
  onApply,
  ...
  const handleInputChange = (field: keyof BlocksUsageFilterValues, value: string) => {
  ...
  return (
    <div className='mb-4 rounded-md bg-gray-50 p-4 dark:bg-slate-800'>
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-7'>
        ...
        {/* Min Entries Filter */}
        <div>
          <label className='mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300'>Min Entries</label>
          <div className='relative'>
            <input
              type='number'
              min='0'
              value={filters.minEntries}
              onChange={(e) => handleInputChange('minEntries', e.target.value)}
              className='block w-full rounded-md border border-gray-300 bg-white px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100'
              placeholder='0'
            />
          </div>
        ...
        {/* Max Entries Filter */}
        <div>
          <label className='mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300'>Max Entries</label>
          <div className='relative'>
            <input
              type='number'
              min='0'
      ...
      {hasActiveFilters && (
        <div className='mt-4 flex flex-wrap gap-2'>
          ...
          {appliedFilters?.status && (
            <span className='inline-flex items-center gap-1 rounded-full bg-indigo-100 px-3 py-1 text-sm text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200'>
              Status: {statusOptions.find((opt) => opt.value === appliedFilters.status)?.label}
              <button
                onClick={() => {
                  handleInputChange('status', '');
                  onApply({ ...filters, status: '' });
                }}
                className='ml-1 hover:text-indigo-600'
              >
                <X className='h-3 w-3' />
          ...
          {appliedFilters?.minEntries && (
            <span className='inline-flex items-center gap-1 rounded-full bg-indigo-100 px-3 py-1 text-sm text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200'>
              Min Entries: {appliedFilters.minEntries}
              <button
                onClick={() => {
                  handleInputChange('minEntries', '');
                  onApply({ ...filters, minEntries: '' });
                }}
                className='ml-1 hover:text-indigo-600'
              >
                <X className='h-3 w-3' />
          ...
          {appliedFilters?.maxEntries && (
            <span className='inline-flex items-center gap-1 rounded-full bg-indigo-100 px-3 py-1 text-sm text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200'>
              Max Entries: {appliedFilters.maxEntries}
              <button
                onClick={() => {
                  handleInputChange('maxEntries', '');
                  onApply({ ...filters, maxEntries: '' });
                }}
                className='ml-1 hover:text-indigo-600'
              >
                <X className='h-3 w-3' />
