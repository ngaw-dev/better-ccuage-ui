...
export interface SessionUsageFilterValues {
  search: string;
  projectPath: string;
  model: string;
  minCost: string;
...
interface SessionUsageFiltersProps {
  filters: SessionUsageFilterValues;
  appliedFilters: SessionUsageFilterValues;
  onChange: (filters: SessionUsageFilterValues) => void;
  onApply: (filters: SessionUsageFilterValues) => void;
  onClear: () => void;
  availableModels: string[];
  availableProjects: string[];
}

export function SessionUsageFilters({
  filters,
  appliedFilters,
  onChange,
  onApply,
  ...
  const handleInputChange = (field: keyof SessionUsageFilterValues, value: string) => {
  ...
  return (
    <div className='mb-4 rounded-md bg-gray-50 p-4 dark:bg-slate-800'>
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-5'>
        ...
        {/* Project Path Filter */}
        <div>
          <label className='mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300'>Project Path</label>
          <SingleSelect
            options={projectOptions}
            value={filters.projectPath}
            onChange={(value) => handleInputChange('projectPath', value)}
      ...
      {hasActiveFilters && (
        <div className='mt-4 flex flex-wrap gap-2'>
          ...
          {appliedFilters?.projectPath && (
            <span className='inline-flex items-center gap-1 rounded-full bg-indigo-100 px-3 py-1 text-sm text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200'>
              Project: {appliedFilters.projectPath}
              <button
                onClick={() => {
                  handleInputChange('projectPath', '');
                  onApply({ ...filters, projectPath: '' });
                }}
                className='ml-1 hover:text-indigo-600'
              >
                <X className='h-3 w-3' />
