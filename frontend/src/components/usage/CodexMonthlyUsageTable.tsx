...
import { CodexMonthlyUsageFilters, CodexMonthlyUsageFilterValues } from './CodexMonthlyUsageFilters';
import { ChevronUp, ChevronDown, ChevronsUpDown, Settings2, FileX } from 'lucide-react';

interface CodexMonthlyUsageTableProps {
  data: NonNullable<UsageData['codexMonthly']>;
}

type SortableField = 'month' | 'inputTokens' | 'outputTokens' | 'cacheReadTokens' | 'totalTokens' | 'totalCost';
type SortOrder = 'asc' | 'desc';

export function CodexMonthlyUsageTable({ data }: CodexMonthlyUsageTableProps) {
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [sortBy, setSortBy] = useState<SortableField>('month');
  const [sortOrder, setSortOrder] = useState<SortOrder>('desc');

  const [appliedFilters, setAppliedFilters] = useState<CodexMonthlyUsageFilterValues>({
    search: '',
    month: '',
    model: '',
    minCost: '',
  ...
  const [localFilters, setLocalFilters] = useState<CodexMonthlyUsageFilterValues>({
    search: '',
    month: '',
    model: '',
    minCost: '',
  ...
  const handleApplyFilters = (filters: CodexMonthlyUsageFilterValues) => {
    setAppliedFilters(filters);
  };

  const handleClearFilters = () => {
    const clearedFilters: CodexMonthlyUsageFilterValues = {
      search: '',
      month: '',
      model: '',
      minCost: '',
  ...
  return (
    <div className='metric-card'>
      <div className='mb-4 flex flex-row items-center justify-between'>
        <h3 className='text-lg font-semibold'>Codex Monthly Usage Report</h3>
        <button
